...
def hog(im, int sbin=8):
    ...
    """
    cdef numpy.ndarray[numpy.double_t, ndim=3] data, feat
    cdef numpy.ndarray[numpy.double_t, ndim=1] hist, norm
    ...
    cdef numpy.ndarray[numpy.double_t, ndim=1] uu
    uu = numpy.array([ 1.0000,  0.9397,  0.7660,  0.500,  0.1736, 
                   -0.1736, -0.5000, -0.7660, -0.9397])
    cdef numpy.ndarray[numpy.double_t, ndim=1] vv
    vv = numpy.array([0.0000, 0.3420, 0.6428, 0.8660, 0.9848, 
    ...
    data = numpy.asarray(im, dtype=numpy.double)

    cc0 = <unsigned int>(0)
    cc1 = <unsigned int>(1)
    cc2 = <unsigned int>(2)

    hist = numpy.zeros(shape=(blocks0 * blocks1 * 9), dtype=numpy.double)
    norm = numpy.zeros(shape=(blocks0 * blocks1), dtype=numpy.double)
    feat = numpy.zeros(shape=(out0, out1, out2), dtype=numpy.double)
...
def rgbhist(im, int binsize=8):
    ...
    cdef numpy.ndarray[numpy.uint8_t, ndim=3] data = numpy.asarray(im)
    cdef numpy.ndarray[numpy.double_t, ndim=1] hist 
    hist = numpy.zeros(binsize * binsize * binsize)
...
def rgbmean(im):
    ...
    cdef double r, g, b
    cdef numpy.ndarray[numpy.uint8_t, ndim=3] data = numpy.asarray(im)
    cdef numpy.ndarray[numpy.double_t, ndim=1] out = numpy.zeros(9)
